public static void MuteAndReportGun()//UND
{
    if (ControllerInputPoller.instance.rightGrab)
    {
        ...
        {
            if (pointRig != null && pointRig != GorillaTagger.Instance.offlineVRRig)
            {
                foreach (GorillaScoreBoard gorillaScoreBoard in GorillaScoreboardTotalUpdater.allScoreboards)
                {
                    ...
                    {
                        if (line.linePlayer == pointRig.OwningNetPlayer)
                        {
            ...
            {
                pointRig = hitInfo.collider.GetComponentInParent<VRRig>();
            }
        }
    }
    if (pointer != null)
